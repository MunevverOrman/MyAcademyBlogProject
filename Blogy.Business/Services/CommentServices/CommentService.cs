using AutoMapper;
using Blogy.Business.DTOs.CommentDtos;
using Blogy.DataAccess.Repositories.CommentRepositories;
using Blogy.Entity.Entities;
using FluentValidation;

namespace Blogy.Business.Services.CommentServices
{
    public class CommentService(ICommentRepository _commentRepository,
        IMapper _mapper,
        IValidator<Comment> _validator) : ICommentService
    {
        public async Task CreateAsync(CreateCommentDto dto)
        {
            var comment=_mapper.Map<Comment>(dto);//dto yu entity ye mapladik
            var result= await _validator.ValidateAsync(comment); //commenti validasyondan geçirdim geriye result döndü
            if (!result.IsValid)//validasyondan gelen result validasyondan geçmediyse
            {
                throw new ValidationException(result.Errors);//5 parametre var 2. sini uyguladık.resultta hatalar var zaten diye.ilk olarak hatatyı fırlattık.
            }
           await _commentRepository.CreateAsync(comment);

        }

        public async Task DeleteAsync(int id)
        {
           await _commentRepository.DeleteAsync(id);
        }

        public async Task<List<ResultCommentDto>> GetAllAsync()
        {
           var values=await _commentRepository.GetAllAsync();
            return _mapper.Map<List<ResultCommentDto>>(values);

        }

        public async Task<UpdateCommentDto> GetByIdAsync(int id)
        {
            var value= await _commentRepository.GetByIdAsync(id);
            return _mapper.Map<UpdateCommentDto>(value);
        }

        public async Task<List<ResultCommentDto>> GetCommentsByUserIdAsync(int userId)
        {
            var values = await _commentRepository.GetCommentsByUserIdAsync(userId);
            return _mapper.Map<List<ResultCommentDto>>(values);
        }

        public async Task<List<ResultCommentDto>> GetCommentsByBlogIdAsync(int blogId)
        {
            var values = await _commentRepository.GetCommentsByBlogIdAsync(blogId);
            return _mapper.Map<List<ResultCommentDto>>(values);
        }


        public async Task<ResultCommentDto> GetSingleByIdAsync(int id)
        {
            var value = await _commentRepository.GetByIdAsync(id);
            return _mapper.Map<ResultCommentDto>(value);
        }

        public async Task UpdateAsync(UpdateCommentDto dto)
        {
            var comment = _mapper.Map<Comment>(dto);//dto yu entity ye mapladik
            var result = await _validator.ValidateAsync(comment); //commenti validasyondan geçirdim geriye result döndü
            if (!result.IsValid)//validasyondan gelen result validasyondan geçmediyse
            {
                throw new ValidationException(result.Errors);//5 parametre var 2. sini uyguladık.resultta hatalar var zaten diye.ilk olarak hatatyı fırlattık.
            }
            await _commentRepository.UpdateAsync(comment);
        }
    }
}
